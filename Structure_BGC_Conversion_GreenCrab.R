...
#LocusMixed2 <- LocusMixed[rep(c(TRUE,FALSE),nrow(CrabMixed_allele)/2)]
...
Loci=gsub("X","",as.character(colnames(MixedStruct1)[3:length(MixedStruct1)]))
colnames(MixedStruct1)[3:length(MixedStruct1)]=Loci

MixedStruct=MixedStruct1[,c(colnames(MixedStruct1)[1:2],LocusMixed)]

MixedStruct%>%group_by(Pop,ID)%>%summarise
...
#This will create the file structure specified by the Mixed File structure data according to the BGC manual
MixedData <- NULL
...
write.table(MixedData,"MixedData_Crab.txt",sep="\t",quote=FALSE,row.names=FALSE,col.names=FALSE)
